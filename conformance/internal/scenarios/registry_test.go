...
// TestRunSuite_ULCWrapperFailureDetailsPreserved verifies that ULC wrapper
// scenario failures preserve diagnostic details (Failure.Actual text).
// This test specifically covers the ULC smoke scenario which uses runProfile
// and pollRunCompletion to handle wrapper failures.
func TestRunSuite_ULCWrapperFailureDetailsPreserved(t *testing.T) {
	// Create a test server that returns a failing response for /runs
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/healthz":
	...
	// Run the ULC smoke scenario which will fail via runProfile against the test server
	scenario := ULCSmokeScenario()
	report := RunSuite(context.Background(), env, []Scenario{scenario}, []string{"ulc.shell.bash"})
	...
	// Verify failure details are preserved (regression check for RF-002)
	if result.Failure == nil {
		t.Error("Expected Failure to be non-nil for failed result")
	} else {
		// Verify Actual is non-empty (critical for actionable failure text)
		...
		// Verify Actual contains error context from the server response
		// Should contain the HTTP status code from runProfile
		if !strings.Contains(result.Failure.Actual, "500") &&
			!strings.Contains(result.Failure.Actual, "internal server error") &&
			!strings.Contains(result.Failure.Actual, "POST /runs returned status") {
			t.Errorf("Expected Failure.Actual to contain error context (status code or body), got: %q", result.Failure.Actual)
		}

		// Verify token redaction is applied (security check)
		if strings.Contains(result.Failure.Actual, "test-token-ulc-wrapper") {
			t.Errorf("Expected token to be redacted from Failure.Actual, but found: %q", result.Failure.Actual)
		}
	}
}
